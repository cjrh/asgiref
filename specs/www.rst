...
The HTTP format covers HTTP/1.0, HTTP/1.1, HTTP/2 and HTTP/3. Protocol
version differences are largely at the transport level. A protocol server
...
* For the ``Set-Cookie`` header in HTTP/1.0, HTTP/1.1, HTTP/2 and HTTP/3, it may
  appear repeatedly, but cannot be concatenated by commas (or anything else)
...
* For the ``Cookie`` header, in HTTP/1.0 and HTTP/1.1, RFC 7230 and RFC 6265
  ...
  and HTTP/3, RFC 9113 and RFC 9114 state that ``Cookie`` headers MAY appear
...
* ``http_version`` (*Unicode string*) -- One of ``"1.0"``, ``"1.1"``, ``"2"`` or
  ``"3"``.
